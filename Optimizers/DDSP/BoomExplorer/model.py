...
from typing import Optional, Dict, Tuple, NoReturn, List, Any, Union, Hashable
...
class MOGP(ExactGP, MultiTaskGPyTorchModel):
    ...
    def __init__(
        ...
        self.num_non_task_features = 0
        self.num_tasks = len(output_tasks)
    ...
    def construct_inputs(
        cls, 
        training_data: Union[SupervisedDataset, 
...
class DKLGP(object):
    def __init__(self, x: torch.Tensor, y: torch.Tensor, **kwargs: Dict):
        ...
        self.gp = MOGP(x.to(torch.float64), y, task_feature=-1, mean_type="linear")
        print("BOOM Explorer DKLGP using:", self.device)
    ...
    def train(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.forward_mlp(x)
        x = self.transform_xlayout(x)
        self.gp.set_train_data(x.to(torch.float64))
        y = self.gp(x.to(torch.float64))
        return y
