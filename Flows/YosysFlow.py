...
from Flows.YosysUtils import read_report
...
SCRIPT_TEMPLATE = "Flows/templates/yosys_synth_template.ys"
SCRIPT_GENERATE = FLOW_BUILD_DIR + '/yosys_synth.ys'

DEFAULT_PDK_LIB_PATH = "Flows/pdk/skywater130.lib"

class YosysFlow(Flow):
    ...
    def evaluate(self, generated_rtl : str, top_name : str, 
        ...
        if multi_files:
            ...
            tcl_str = template_str.format(
                ...
                TOP_NAME = top_name,
                PDK = self.pdk_path,
                CLOCK_PERIOD=clk, # Unused
                CLOCK_NAME=clk_name, # Unused
                REPORT_PATH= FLOW_REPORT_DIR
            )

            with open(SCRIPT_GENERATE, 'w') as tcl:
                tcl.write("# This file is automatically generated at ")
                tcl.write(time.asctime(time.localtime(time.time()))+"\n")
                tcl.write(tcl_str)
        cmd = f"yosys {SCRIPT_GENERATE} > {FLOW_BUILD_DIR}/yosys.log"
        print(cmd)
        ret = os.system(cmd)
        ...
        if ret==0:
            result = {}
            result.update(read_report(f"{FLOW_REPORT_DIR}/yosys.rpt"))
            return result
        else:
...
#     ysflow = YosysFlow()
#     res = ysflow.evaluate("Design/riscv/VexiiRiscv/VexiiRiscv.v",
#                           "VexiiRiscv", clk = 10)
#     print(res)